...
function app_link_custom (platforms, fallbackUrl) {
  ...
  app_link_custom.platforms = platforms;
  app_link_custom.fallbackUrl = fallbackUrl;
  // Exexcute any before hooks.
  if (app_link_custom.before && !isRedirectPage) {
    return app_link_custom.before(function() {
      app_link_custom.before = null;
      app_link_custom(app_link_custom.platforms, app_link_custom.fallbackUrl);
    });
  }
  // Add referrer to fallback (Server is unable to do this accurately).
  var platform = platforms.app_link_custom_platform_fallback;
  if (platform && platform.supports_qs) {
    app_link_custom.fallbackUrl = app_link_custom.applyReferrer(app_link_custom.fallbackUrl);
  }
  // Determine the user's platform.
  var platformKey = app_link_custom.getPlatformKey(navigator.userAgent) || app_link_custom.getPlatformKey(navigator.appVersion);
  ...
  if (!platform) {
    return app_link_custom.fallback();
  }
  // Transform App Urls according to rules.
  var appUrl = app_link_custom.transformUrl(platform.app_url, platform.supports_qs, platform.supports_path, platform.path_whitelist);
  var intentUrl = app_link_custom.transformUrl(platform.intent_url, platform.supports_qs, platform.supports_path, platform.path_whitelist);
  // We have a platform, update fallback to be the store.
  app_link_custom.fallbackUrl = app_link_custom.transformUrl(platform.store_url, platform.supports_store_qs) || fallbackUrl;

  var UA = navigator.userAgent;

  // Android w/ Chrome must use Intents
  if (platformKey === 'app_link_custom_platform_android' && UA.match(/Chrome/)) {
    return app_link_custom.directOrIframe(platform.app_url);
  }
  // Firefox likes direct.
  else if (platformKey === 'app_link_custom_platform_android' && UA.match(/Firefox/)) {
    return app_link_custom.directOrIframe(appUrl);
  }
  // iOS9 needs middle redirect and confirm handling.
  // The dialog flow is not pretty - sorry.
  else if ((platformKey === 'app_link_custom_platform_iphone' || platformKey === 'app_link_custom_platform_ipad') && app_link_custom.getOS(UA) >= 9) {
    if (isRedirectPage) {
      return app_link_custom.fallback();
    }
    else {
      return app_link_custom.directOrRedirect(appUrl);
    }
  }
  // iOS8 Chrome prefers directOrIframe trick.
  else if((platformKey === 'app_link_custom_platform_iphone' || platformKey === 'app_link_custom_platform_ipad') &&  UA.match(/CriOS/)) {
    return app_link_custom.directOrIframe(appUrl);
  }
  // Most iOS & Android: Prefers hidden iframe trick over direct.
  else {
    return app_link_custom.iframe(appUrl);
  }
}
 ...
 */
app_link_custom.getPlatformKey = function (UA) {
  return (
    UA.match(/Windows Phone/i) ? 'app_link_custom_platform_windows_phone' :
    UA.match(/Kindle/i) ? 'app_link_custom_platform_kindle_fire' :
    UA.match(/Android/i) ? 'app_link_custom_platform_android' :
    UA.match(/iPhone|iPod/i) ? 'app_link_custom_platform_iphone' :
    UA.match(/iPad/i) ? 'app_link_custom_platform_ipad' :
    (UA.match(/Windows NT/) && app_link_custom.getOS(UA) >= 6.2) ? 'app_link_custom_platform_windows' :
    (UA.match(/OS X/) && app_link_custom.getOS(UA) >= 10.6) ? 'app_link_custom_platform_mac' :
    ''
  );
};
 ...
 */
app_link_custom.getOS = function (UA) {
  var version;
  // Windows
 ...
 *   The transformed URL.
 */
app_link_custom.transformUrl = function (url, applyQs, applyPath, pathWhitelist) {
  if (url && applyPath) {
    var path = app_link_custom.getQueryParams().path;
    if (app_link_custom.isPathWhitelisted(path, pathWhitelist)) {
      url = app_link_custom.applyUrl(url, {pathname: path});
    }
  }
  if (url && applyQs) {
    // Pass-Through Query-String values.
    url = app_link_custom.applyUrl(url, {search: location.search});
    // Attempt to set a referrer.
    url = app_link_custom.applyReferrer(url);
  }
  return url;
};
 ...
 */
app_link_custom.isPathWhitelisted = function (path, whitelist) {
 ...
 */
app_link_custom.getQueryParams = function () {
  var query = {};
  (location.search || "")
    // Remove the "?"
    .substr(1)
 ...
 *   The transformed URL.
 */
app_link_custom.applyReferrer = function (url) {
  if (url && document.referrer && url.indexOf("referrer") === -1) {
    return app_link_custom.applyUrl(url, {search: "referrer=" + encodeURIComponent(document.referrer)});
  }
  return url;
};
 ...
 *   The transformed URL.
 */
app_link_custom.applyUrl = function (url, parts) {
 ...
 */
app_link_custom.onFailure = function (callback) {
 ...
 *   The URL directed to.
 */
app_link_custom.directOrIframe = function (url) {
  if (!url) {
    return app_link_custom.fallback();
  }
  // If we're still here, try the iframe trick.
  app_link_custom.onFailure(function() {
    app_link_custom.iframe(url);
  });
 ...
 *   The URL directed to.
 */
app_link_custom.directOrRedirect = function (url) {
  if (!url) {
    return app_link_custom.fallback();
  }
  // If we're still here, try the iframe trick.
  app_link_custom.onFailure(function() {
    ...
    document.location = href + (href.match(/\?/) ? "&" : "?") + "app_link_custom_redirect=true";
  });
  document.location = url;
  return url;
};
 ...
 *   The URL directed to.
 */
app_link_custom.iframe = function (url) {
  if (!url) {
    return app_link_custom.fallback();
  }
  // If we're still here, then give up and go to fallback.
  app_link_custom.onFailure(app_link_custom.fallback);
  var iframe = document.createElement("iframe");
  iframe.style.border = "none";
  iframe.style.width = "1px";
  iframe.style.height = "1px";
  iframe.onload = app_link_custom.fallback;
  iframe.onerror = app_link_custom.fallback;
  iframe.src = url;
 ...
 */
app_link_custom.fallback = function () {
  if (!document.webkitHidden && !document.hidden) {
    window.location = app_link_custom.fallbackUrl;
    return app_link_custom.fallbackUrl;
  }
  else {
    window.close();
  }
