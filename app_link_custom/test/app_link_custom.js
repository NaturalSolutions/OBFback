...
describe("App Link Redirects", function() {
      ...
      evaluate = evaluate.then(function() {
        return new Promise(function (resolve, reject) {
          ...
          var browserCall = function() {
            ...
            try {
              return app_link_custom.apply(this, arguments);
            }
            catch (err) {
              location.href = "";
              return {
          ...
          page.evaluate(browserCall, nodeReceive,
            ...
            applink, applink.app_link_custom_platform_fallback.fallback_url
          );
        });
      });
  ...
  it("Unsupported Desktop to the Fallback.", function(done) {
    testApplinkRedirect({}, applink.app_link_custom_platform_fallback.fallback_url, done);
  });
  ...
  testPlatform("iPhone", "app_link_custom_platform_iphone");
  testPlatform("iPod", "app_link_custom_platform_iphone");
  testPlatform("iPad", "app_link_custom_platform_ipad");
  testPlatform("Android Browser", "app_link_custom_platform_android");
  testPlatform("Android Chrome", "app_link_custom_platform_android");
  testPlatform("Windows Phone", "app_link_custom_platform_windows_phone");
  testPlatform("Kindle Fire", "app_link_custom_platform_kindle_fire");
  testPlatform("OSX Safari", "app_link_custom_platform_mac");
  testPlatform("OSX Chrome", "app_link_custom_platform_mac");
  testPlatform("OSX Firefox", "app_link_custom_platform_mac");
  testPlatform("Windows Explorer", "app_link_custom_platform_windows");
  testPlatform("Windows Chrome", "app_link_custom_platform_windows");
  testPlatform("Windows Firefox", "app_link_custom_platform_windows");
  ...
  function testProcessing (browser, platformKey) {
    ...
    describe("Processing " + urlType, function() {
      it("Fallbacks back w/o App URL or Store URL.", function(done) {
        ...
        testApplinkRedirect({browser: browser}, applink.app_link_custom_platform_fallback.fallback_url, done);
      });
  ...
  testProcessing("iPhone", "app_link_custom_platform_iphone");
  testProcessing("Android Chrome", "app_link_custom_platform_android");
  ...
  //   applink.app_link_custom_platform_fallback.supports_qs = 1;
  //   testApplinkRedirect(applink, {baseURL: BASE_URL + "?a=b&c=d"},
  //     applink.app_link_custom_platform_fallback.fallback_url + "?a=b&c=d",
  //     done
  //   );
  // });
  ...
  describe("Before Hook", function() {
    it("Modifies fallback variable.", function(done) {
      testApplinkRedirect({inject: function() {
        app_link_custom.before = function(cb){
          app_link_custom.fallbackUrl += "?a=b";
          cb();
        };
      }}, applink.app_link_custom_platform_fallback.fallback_url + "?a=b", done);
    });
    it("Modifies platform variables.", function(done) {
      testApplinkRedirect({browser: "iPhone", inject: function() {
        app_link_custom.before = function(cb){
          app_link_custom.platforms.app_link_custom_platform_iphone.app_url += "?a=b";
          cb();
        };
      }}, applink.app_link_custom_platform_iphone.app_url + "?a=b", done);
    });
    it("Wait for async.", function(done) {
      testApplinkRedirect({inject: function() {
        app_link_custom.before = function(cb){ setTimeout(cb, 33); };
      }}, applink.app_link_custom_platform_fallback.fallback_url, done);
    });
    it("Should wait indefinitely for broken async.", function(done) {
      testApplinkRedirect({inject: function() {
        app_link_custom.before = function(){ };
      }}, applink.app_link_custom_platform_fallback.fallback_url, function (err) {
        if (err) {
          done();
        } else {
